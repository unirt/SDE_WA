    ...
    case 5:
      ...
      if (X->alg == N_V) {                         /* 5th RK for N_V */
        if (jth == 0 && X->sde->sde_type == ITO) { /* V_0 && Ito form */
          ...
          for (i = 0; i < X->sde->dim_y; i++)
            Y2[i] = X->initv[i] + (11.0 / 64.0) * s * fY0[i] +
                    (5.0 / 64.0) * s * fY1[i];
          ...
          for (i = 0; i < X->sde->dim_y; i++)
            Y4[i] = X->initv[i] + (3.0 / 64.0) * s * fY0[i] -
                    (15.0 / 64.0) * s * fY1[i] + (3.0 / 8.0) * s * fY2[i] +
                    (9.0 / 16.0) * s * fY3[i];
          Ito_to_Strt_drift(X, Y4, fY4);
          for (i = 0; i < X->sde->dim_y; i++)
            Y5[i] = X->initv[i] + (5.0 / 7.0) * s * fY1[i] +
                    (6.0 / 7.0) * s * fY2[i] - (12.0 / 7.0) * s * fY3[i] +
                    (8.0 / 7.0) * s * fY4[i];
          Ito_to_Strt_drift(X, Y5, fY5);
          for (i = 0; i < X->sde->dim_y; i++)
            X->destv[i] =
                X->initv[i] + (7.0 / 90.0) * s * fY0[i] +
                (32.0 / 90.0) * s * fY2[i] + (12.0 / 90.0) * s * fY3[i] +
                (32.0 / 90.0) * s * fY4[i] + (7.0 / 90.0) * s * fY5[i];
        ...
        for (i = 0; i < X->sde->dim_y; i++)
          X->destv[i] = X->initv[i] + (7.0 / 90.0) * fY0[i] +
                        (32.0 / 90.0) * fY2[i] + (12.0 / 90.0) * fY3[i] +
                        (32.0 / 90.0) * fY4[i] + (7.0 / 90.0) * fY5[i];
      }
      break;
    case 7: {
      ...
      if (X->alg == 1) {                           /* 7th RK for N_V */
        if (jth == 0 && X->sde->sde_type == ITO) { /* V_0 && Ito form */
          ...
          Ito_to_Strt_drift(X, Y3, fY3);
          for (i = 0; i < X->sde->dim_y; i++)
            Y4[i] = X->initv[i] + a51 * s * fY0[i] + a53 * s * fY2[i] +
                    a54 * s * fY3[i];
          Ito_to_Strt_drift(X, Y4, fY4);
          for (i = 0; i < X->sde->dim_y; i++)
            Y5[i] = X->initv[i] + a61 * s * fY0[i] + a63 * s * fY2[i] +
                    a64 * s * fY3[i] + a65 * s * fY4[i];
          Ito_to_Strt_drift(X, Y5, fY5);
          for (i = 0; i < X->sde->dim_y; i++)
            Y6[i] = X->initv[i] + a71 * s * fY0[i] + a73 * s * fY2[i] +
                    a74 * s * fY3[i] + a75 * s * fY4[i] + a76 * s * fY5[i];
          ...
          for (i = 0; i < X->sde->dim_y; i++)
            Y8[i] = X->initv[i] + a91 * s * fY0[i] + +a93 * s * fY2[i] +
                    a94 * s * fY3[i] + a95 * s * fY4[i] + a96 * s * fY5[i] +
                    a97 * s * fY6[i] + a98 * s * fY7[i];
          Ito_to_Strt_drift(X, Y8, fY8);
          for (i = 0; i < X->sde->dim_y; i++)
            X->destv[i] = X->initv[i] + b4 * s * fY3[i] + b5 * s * fY4[i] +
                          b6 * s * fY5[i] + b7 * s * fY6[i] + b8 * s * fY7[i] +
                          b9 * s * fY8[i];
        } else { /* V_1, ... V_d, or Stratonovich form */
          pf = X->sde->V[jth](X->initv, fY0, X->sde->params);
          for (i = 0; i < X->sde->dim_y; i++)
        ...
        pf = nn_vf_W(X, s, X->initv, fY0, jth);
        for (i = 0; i < X->sde->dim_y; i++) Y1[i] = X->initv[i] + a21 * fY0[i];
        pf = nn_vf_W(X, s, Y1, fY1, jth);
        for (i = 0; i < X->sde->dim_y; i++) Y2[i] = X->initv[i] + a32 * fY1[i];
        pf = nn_vf_W(X, s, Y2, fY2, jth);
        for (i = 0; i < X->sde->dim_y; i++)
