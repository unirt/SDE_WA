...
SDE_WA_SYSTEM *alloc_SDE_WA_SYSTEM(int N, int d, void *par) {
  ...
  sde->V = (int (**)(const double y[], double dy[], void *params))malloc(
      sizeof(int (*)(const double y[], double dy[], void *params)) * (1 + d));
  sde->drift_corrector =
      (int (**)(const double y[], double *dVdy[], void *params))malloc(
          sizeof(int (*)(const double y[], double *dVdy[], void *params)) *
          (1 + d));
  sde->exp_sV = (int (**)(
      double s, const double y[], double exp_sVy[],
      void *params))malloc(sizeof(int (*)(double s, const double y[],
                                          double exp_sVy[], void *params)) *
                           (1 + d));
...
SDE_WA_SLTN *alloc_SDE_WA_SLTN(enum ALG alg, int mth_is, SDE_WA_SYSTEM *sde) {
  ...
  sltn->drift_corrector_matrix =
      (double **)malloc(sizeof(double *) * sde->dim_y);
  sltn->drift_step_interv = (double *)malloc(sizeof(double) * (sde->dim_y));
  for (j = 0; j < sde->dim_y; j++)
    sltn->drift_corrector_matrix[j] =
        (double *)malloc(sizeof(double) * sde->dim_y);
  switch (alg) {
    case 0:
    ...
    case 1:
      ...
      if (sltn->mth_is == 5)
        sltn->rk_step_interv =
            (double *)malloc(sizeof(double) * (sde->dim_y) * 11);
      else
        sltn->rk_step_interv =
            (double *)malloc(sizeof(double) * (sde->dim_y) * 17);
      sltn->sample_pt.nv = (RV_NV *)malloc(sizeof(RV_NV));
      sltn->sample_pt.nv->rv_nv_n =
          (double *)malloc(sizeof(double) * (sde->dim_BM));
      break;
    case 2:
      ...
      sltn->nn_sample_pt_interv =
          (double *)malloc(sizeof(double) * (2 * sde->dim_BM));
      break;
  }
  return sltn;
}
  ...
  for (j = 0; j < X->sde->dim_y; j++) {
    ...
    for (i = 1; i <= X->sde->dim_BM; i++) {
      ...
      for (k = 0; k < X->sde->dim_y; k++) {
        sum_tmp +=
            0.5 * X->drift_step_interv[k] * (X->drift_corrector_matrix[j][k]);
      } /* for (k) */
    }   /* for (i) */
